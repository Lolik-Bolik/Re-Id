...
def save_file(weigths,metric_name, name, content):
    """Decode and store a file uploaded with Plotly Dash."""
    clear_uploaded_folder()
    data = content.encode("utf8").split(b";base64,")[1]
    with open(os.path.join(UPLOAD_DIRECTORY, name), "wb") as fp:
        fp.write(base64.decodebytes(data))
    fig = inference(opts, weigths, metric_name, is_plotly=True)
    return fig
...
def update_output(uploaded_filenames, uploaded_file_contents, weights, metric_name):
    """Save uploaded files and regenerate the file list."""
    print('Metric',metric_name)
    if uploaded_filenames is not None and uploaded_file_contents is not None:
        for name, data in zip(uploaded_filenames, uploaded_file_contents):
            return dcc.Graph(figure=save_file(weights,metric_name,name, data))
